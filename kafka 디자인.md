...
- 카프카는 멀티 브로커에 대해 리플리케이션을 적용할 수 있다
  - 사실상 리플리케이션을 적용할 게 아니라면 멀티 브로커를 쓸 이유가 없다
...
- 리플리케이션 팩터란 내 자신을 `복제할 곳의 개수`를 말한다
...
- 리플리케이션이 무조건 좋은 것만은 아니다
  - 리플리케이션이 되는 파티션의 개수만큼 저장소의 용량이 더 필요하게 된다
    ...
    - 팔로워 상태체크, 리더로부터의 데이터 복제 등 리소스 사용량을 어느정도 증가시키게 된다
...
- 리더가 되살아 날 때 까지 기다리는 방법
  - 메시지 손실이 없다(리더가 죽었다면 더 이상 메시지를 받지 못했을 것이기 때문에 더 이상 받은 메시지가 없기 때문)
...
- 지금까진 리플리케이션을 전부 브로커 레벨에서 설명했지만, 실제로는 파티션 단위로 일어난다
  - 사실상 토픽이란 논리적인 단위이기 때문에...
- ![카프카 리플리케이션](img/kafka_replication.jpg)
- 당연한 소리지만 브로커끼리 파티션의 개수는 다 맞춰야한다
