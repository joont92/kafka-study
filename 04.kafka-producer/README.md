# 파티션 키 지정
- 멀티 파티션에서 파티션 키를 지정하지 않고 메시지를 보내면, 여러 파티션에 나뉘어져 저장된다(라운드 로빈?)
- 메시지 키를 지정하면 **같은 키를 가진 메시지는 모두 같은 파티션에 저장된다**
    - 해시 함수와 같은 방식이다
- 같은 키를 가진 메시지끼리의 순서는 무조건 보장할 수 있다
    - 같은 파티션에 저장되기 때문이다
- 그러므로 파티션을 여러개 유지하면서 메시지의 순서를 유지하고 싶다면, 메시지 전송 시 partition key 를 지정해줘야 한다
    - 전체 순서는 중요하지 않고, resource id에 대한 순서만 보장해도 될 경우에만 해당한다(대부분이 이에 해당)
    - resource id에 상관없이 모든 메시지의 순서가 정확하게 보장되어야 한다면 파티션을 1개밖에 사용하지 못한다
...
producer가 해당 엔드포인트에 topic 이름, partition 번호를 전달하면, 요청을 받은 브로커가 전달받은 topic + partition의 리더 브로커 url을 찾아서 반환해준다  
(리더 브로커의 url을 받은 producer는 해당 브로커와 다시 연결하고, 메시지를 전송하게 된다)  
그러므로 모든 브로커의 url을 다 입력할 필요는 없다  
하지만 엔드포인트로 쓴 브로커에 장애가 발생할 경우 클러스터에 요청을 전달할 게이트웨이가 막히는 꼴이 되므로, 카프카에서는 브로커 리스트를 전체 다 입력하는 것을 권장한다고 한다  
...
#### compression.type
프로듀서가 데이터를 압축해서 보낼 수 있는데, 어떤 타입으로 압축할지 정하는 옵션이다  
...
프로듀서에서 데이터를 배치로 보낼 때 사용하는 옵션들이다  
...
> 브로커에서 메시지를 받아도 `min.insync.replicas` 의 개수가 충족되지 않기 때문에 프로듀서로 acks 를 보내주지 못하는 상황이 발생하기 때문이다  